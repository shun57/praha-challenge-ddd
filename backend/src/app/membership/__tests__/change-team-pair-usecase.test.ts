...
describe('do', () => {
  let useCase: ChangeTeamPairUseCase;
  let membershipRepo: IMembershipRepository;

  const param = { membershipId: "12345", newPairId: "12345" }
  ...
  let updatedMembershipEntity = Membership.create({
    ...
    pairId: PairId.create(new UniqueEntityID(param.newPairId)),
    enrollmentStatus: MembershipEnrollmentStatus.create({ value: "在籍中" }),
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<ChangeTeamPairUseCase>(ChangeTeamPairUseCase);
